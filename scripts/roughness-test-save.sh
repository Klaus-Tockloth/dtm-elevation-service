...
# Kürze das Data-Feld für jedes Element im Roughnesses-Array
echo "$curl_response" | jq '(.Attributes.Roughnesses[]?.Data |= (.[:48] + " ..."))'
echo "---------------------------------------------"
# --- End print truncated JSON response ---

# Überprüfen, ob das Attributes.Roughnesses Array existiert und nicht leer ist
if ! echo "$curl_response" | jq -e '.Attributes.Roughnesses | arrays and length > 0' > /dev/null; then
    echo "Fehler: JSON-Antwort enthält kein nicht-leeres 'Attributes.Roughnesses' Array." >&2
...
echo "Verarbeite Roughnesses aus der Antwort ..."

# Parse die JSON-Antwort mit jq, um die base64-Daten, den Kachel-Index und den Origin für jedes Objekt zu extrahieren.
# .Attributes.Roughnesses[]: Iteriert über jedes Element des Roughnesses-Arrays.
# |: Leitet das Ergebnis an den nächsten jq-Ausdruck weiter.
# "\(.Data) \(.TileIndex) \(.Origin)": Erstellt eine einzelne Zeichenkette pro Roughness,
...
echo "$curl_response" | jq -r '.Attributes.Roughnesses[] | "\(.Data) \(.TileIndex) \(.Origin)"' | \
while read -r roughness_data roughness_tile_index roughness_origin; do
  # Überprüfen, ob die extrahierten Variablen nicht leer sind
  if [ -z "$roughness_data" ] || [ -z "$roughness_tile_index" ] || [ -z "$roughness_origin" ]; then
    echo "Warnung: Konnte Data, TileIndex oder Origin für ein Roughness-Objekt nicht extrahieren. Überspringe." >&2
  ...
  output_filename="${roughness_tile_index}.${roughness_origin}.tri.png"

  echo "Verarbeite TileIndex: $roughness_tile_index, Origin: $roughness_origin"
  echo "Speichere Daten in: $output_filename"

  # Dekodiere die base64-Daten und speichere sie als Binärdatei (das PNG-Bild).
  echo "$roughness_data" | base64 -d > "$output_filename"
...
if [ ${PIPESTATUS[0]} -ne 0 ]; then
  echo "Fehler beim Ausführen von jq zum Extrahieren der Roughness-Daten. Überprüfen Sie die jq-Syntax und die JSON-Struktur." >&2
  # Der vollständige Antwort-Body wurde bereits oben ausgegeben, falls nötig.
  exit 1
fi

echo "Alle verfügbaren Roughnesses verarbeitet."
